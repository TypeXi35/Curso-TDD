...
class FakeServer:
    ...
    def __call__(self, *args, **kwargs):
        # Make the Sync Manager think that a new connection was created.
        return self
    
    def send(self, data):
        # Track any command that was send to the server
    ...
    def recv(self, *args, **kwargs):
        if self.last_command == "dummy":
            ...
            return "#ERROR", ValueError("%s - %r" %(
                self.last_command, self.last_message
            ))
...
class TestConnection(unittest.TestCase):
    ...
    def test_exchange_with_server(self):
        with unittest.mock.patch("multiprocessing.managers.listener_client", new ={"pickle": (None, FakeServer())}):
